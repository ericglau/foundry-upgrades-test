...
library Upgrades {
  ...
  function deployUUPSProxy(string memory contractName, bytes memory data, mapping(string => string) storage opts) internal returns (ERC1967Proxy) {
    address impl = deployImplementation(contractName);

    Vm vm = Vm(CHEATCODE_ADDRESS);

    if (equals(opts[Options.usePlatformDeploy], 'true')) {
      console.log("MAPPING: Using platform for deployUUPSProxy");
